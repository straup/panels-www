...
function panels_load_panels(){panels_storage_list(function(a){var c=a.length;if(c){for(var b='<option value="-1">...</option>',d=/^pending_(.*)/,e=0;e<c;e++)if(!a[e].match(d)){b+='<option value="';b+=a[e];b+='">';b+=a[e];b+="</option>"}a=document.getElementById("load-sel");a.innerHTML=b;a.style.display="block"}else alert("Nothing to load")})}function panels_open_canvas(a){var c=a.value;panels_storage_load(c,function(b){panels_new_panel(b,c)})}
function panels_delete_panel(a){a||(a=$("#editor").attr("data-panel-title"));if(a){if(!panels_ui_confirm("Are you sure you want to delete "+a+"?"))return false;panels_storage_remove(a,function(){panels_load_panels()})}else if(!panels_ui_confirm("Are you sure you want to delete this?"))return false;panels_new_panel()}
