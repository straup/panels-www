...
function panels_is_empty(a){if(!a)return true;if(typeof a=="string"&&a=="[]")return true;if(typeof a=="object"&&a.length==0)return true;return false};function panels_utils_data_uri_to_blob(a){var c;c=a.split(",")[0].indexOf("base64")>=0?atob(a.split(",")[1]):unescape(a.split(",")[1]);a=a.split(",")[0].split(":")[1].split(";")[0];for(var b=new Uint8Array(c.length),d=0;d<c.length;d++)b[d]=c.charCodeAt(d);return new Blob([b],{type:a})};function panels_storage_save(a,c,b){var d=(new Date).getTime();c.created=d;localforage.setItem(a,c,function(e){b&&b(e)})}function panels_storage_load(a,c){localforage.getItem(a,function(b){c&&c(b)})}function panels_storage_remove(a,c){localforage.removeItem(a,function(b){c&&c(b)})}function panels_storage_list(a){localforage.keys(a)};function panels_uploads_configured(){return typeof panels_custom_prepare_upload!="function"?true:false}
function panels_uploads_schedule_upload(a){if(typeof panels_custom_prepare_upload!="function"){panels_ui_error("Uploads are not configured");return false}var c=window.btoa((new Date).toISOString()),b="pending_"+c;localforage.setItem(b,a,function(){localforage.getItem(b,function(){try{var d=panels_utils_data_uri_to_blob(a)}catch(e){panels_ui_error("Ack! There was a problem preparing your sketch for uploading");console.log(e);return false}panels_custom_prepare_upload(d,function(f){panels_uploads_schedule_make_it_so(f.endpoint,
f.formdata,c)});return true})});return true}function panels_uploads_schedule_make_it_so(a,c,b){var d=new XMLHttpRequest;d.open("POST",a,true);d.onload=function(){if(d.status==200){typeof panels_custom_upload_ok=="function"&&panels_custom_upload_ok(d);var e="pending_"+b;localforage.removeItem(e,function(){console.log("removed "+e)})}else console.log(e)};d.onerror=function(){panels_ui_error("Hrm... there was a problem contacting the remote server.")};d.send(c);return false};
