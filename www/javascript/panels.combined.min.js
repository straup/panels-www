...
function panels_new_panel(a,c){if(sketchpad){var b=sketchpad.json();if(!panels_is_empty(b))if(!panels_ui_confirm("You've started a sketch. Do you want to overwrite it?"))return false}b={width:window.innerWidth,height:window.innerHeight,editing:true};if(a){try{var d=JSON.parse(a.data);b.strokes=d}catch(e){panels_ui_error("Failed to load that document because "+e);panels_ui_confirm("Should it be removed?")&&panels_delete_panel(c)}if(a.width&&a.width>b.width)b.width=a.width;if(a.height&&a.height>b.height)b.height=
a.height}$("#editor").children().remove();sketchpad=Raphael.sketchpad("editor",b);d=sketchpad.pen();d.width(1);d.opacity(0.6);if(c){$("#editor").attr("data-panel-title",c);$("#header-delete").show();$("#header-show").hide()}else{$("#editor").removeAttr("data-panel-title");$("#header-delete").hide()}$("#header-source").show();$("#header-editor").hide()}
